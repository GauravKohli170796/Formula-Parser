...
export function getValidationErrors(tokens: Token[], supportedRefs?: string[]) {
  ...
  for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
    ...
    if (operatorRequiredMap[token.type] && prev && ![...valueAllowedAfter,TokenType.SquareBracketStart,TokenType.ConditionBracketStart].includes(prev.type)) {
    ...
    if (token.type === TokenType.Comma) {
      if ((functionLevel <= 0 && unclosedTokens[unclosedTokens.length-1].token.type!== TokenType.SquareBracketStart) || !prev || !operatorAllowedAfter.includes(prev.type)) {
    ...
    if (token.type === TokenType.ElseCondition) {
      ...
      if(requiredMissingTokens[requiredMissingTokens.length -1]?.tokenType === TokenType.ElseCondition){
    ...
    if(token.type === TokenType.ConditionBracketEnd){
      ...
      if (unclosedTokens.length && unclosedTokens[unclosedTokens.length - 1].type === TokenType.ConditionBracketStart) {
        ...
        else if([TokenType.IfCondition,TokenType.ElseIfCondition,TokenType.ElseCondition].includes(lastConditionalBracketToken) && prev?.type !== TokenType.SquareBracketEnd){
    ...
    if (token.type === TokenType.BracketStart) {
      ...
      else if (prev && ![...valueAllowedAfter,TokenType.IfCondition,TokenType.ElseIfCondition,TokenType.ConditionBracketStart].includes(prev.type)) {
    ...
    if (token.type === TokenType.BracketEnd) {
      ...
      else if (unclosedTokens.length && unclosedTokens[unclosedTokens.length - 1].type === TokenType.ElseIfCondition) {
        unclosedTokens.pop();
        ifConditionTokens.pop();
        if(prev && prev.type === TokenType.BracketStart){
    ...
    if (token.type === TokenType.SquareBracketEnd) {
      if (unclosedTokens.length && unclosedTokens[unclosedTokens.length - 1].type === TokenType.SquareBracketStart) {
        unclosedTokens.pop()
        if (!prev || (!operatorAllowedAfter.includes(prev.type))) {
          addError(ErrorType.UnexpectedSquareBracket)
        }
    ...
    if (token.type === TokenType.ReferenceVariableStart) {
      ...
      if (prev && ![...valueAllowedAfter,TokenType.SquareBracketStart,TokenType.ConditionBracketStart].includes(prev.type)) {
        addError(ErrorType.OperatorRequiredBeforeReference)
      }
    }

    if (token.type === TokenType.ReferenceName && token.value) {
      if(ifConditionTokens.length>0 && requiredMissingTokens[requiredMissingTokens.length-1]?.tokenType !== TokenType.ElseCondition){
           requiredMissingTokens.push({token:tokens[ifConditionTokens[ifConditionTokens.length-1]],tokenType: TokenType.ElseCondition, errorType: ErrorType.Required})
      }
  ...
  requiredMissingTokens.forEach((requiredMissingToken) => {
      errors.push({token: requiredMissingToken.token , tokenIndex:-1, errorType: requiredMissingToken.errorType,message:`${requiredMissingToken.errorType} "${requiredMissingToken.tokenType}" for "${requiredMissingToken.token?.value}"`})
  })
  return errors;
}